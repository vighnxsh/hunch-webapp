...
You can think of Actions as an API endpoint that will return metadata and something for a user to sign (either a transaction or an authentication message) with their blockchain wallet.
...
Actions also support some level of invalidation before execution. The GET and POST request may return some metadata that states whether the action is capable of being taken (like with the disabled field).
...
Not all blink client applications (e.g. websites or dApps) will support all Actions. Application developers may choose which Actions they want to support within their blink interfaces.
...
For platforms other than HTML/web based ones (like native mobile), the equivalent native user input component should be used to achieve the equivalent experience and client side validation as the HTML/web input types described above.
